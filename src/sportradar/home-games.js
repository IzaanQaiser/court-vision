...
function formatGameDisplay(game, summary, displayTimeZone) {
  const date = game.scheduled ? new Date(game.scheduled) : null;
  const dateStr = date ? formatDateInTimeZone(date, displayTimeZone) : "Unknown";
  const timeStr = date
  ...
  const line = tipoffLabel
    ? `${dateStr} | ${timeStr} | ${away} @ ${home} | ${tipoffLabel}`
    : periodPart
      ? `${dateStr} | ${timeStr} | ${away} @ ${home} | ${scoreLabel} | ${periodPart}`
      : `${dateStr} | ${timeStr} | ${away} @ ${home} | ${scoreLabel}`;
  ...
  return {
    ...
    home,
    status,
    score: tipoffLabel ? null : scoreLabel,
    period: tipoffLabel ? null : periodPart || null,
    tipoff: tipoffLabel,
    line,
  };
}
...
async function fetchHomeGames({ limit = 5 } = {}) {
  const apiKey =
  ...
  return summaries.map(({ game, summary }) =>
    formatGameDisplay(game, summary, displayTimeZone)
  );
}
